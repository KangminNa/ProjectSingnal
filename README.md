...
| 패턴 | 위치 | 역할 |
|------|------|------|
| **Ports & Adapters** | `domain/ports/` ↔ `infrastructure/` | 프로토콜 격리. Port는 순수 인터페이스, Adapter는 구현체 |
| **Pipeline** | `application/events/` → `application/delivery/` | Ingest → Route → Match → Dispatch → Transport → Log |
| **Strategy + Registry** | `infrastructure/transport/` + `application/delivery/transport.registry.ts` | 배달 채널별 분기 (WebSocket, Webhook, Push, Email) |
| **CQRS** | 모든 `application/*/` | Command(상태변경) / Query(읽기전용) 서비스 분리 |
| **Outbox** | `application/events/outbox-relay.service.ts` | DB 저장 후 비동기 Relay로 이벤트 유실 방지 |
...
- `api/` → `application/` 호출
- `application/` → `domain/` Port만 의존 (프로토콜 모름)
- `infrastructure/` → `domain/` Port를 구현 (프로토콜 알음)
- 역방향 의존 불가
...
```
src/
├── domain/                          ① 순수 설계도 (Interface만, 실행코드 없음)
│   ├── entities/                      Project, Consumer, Subscription, ...
│   ├── ports/
│   │   ├── inbound/                   유스케이스 계약
│   │   └── outbound/                  EventBus, TransportAdapter, JobQueue, ...
│   │       └── repositories/          데이터 접근 계약
│   └── types/                         EventEnvelope, ConsumerType, DeliveryResult
│
├── infrastructure/                  ② 프로토콜 구현체
│   ├── persistence/                   PostgreSQL (Drizzle + Schema + Repositories)
│   ├── messaging/                     NATS EventBus 어댑터
│   ├── cache/                         Redis Presence 어댑터
│   ├── queue/                         BullMQ JobQueue 어댑터
│   └── transport/                     배달 프로토콜 (Strategy 구현체)
│       ├── socketio/                    WebSocket (Gateway + Session + Adapter)
│       ├── webhook/                     HTTP Callback
│       ├── push/                        FCM/APNs (stub)
│       └── email/                       SMTP (stub)
│
├── application/                     ③ 비즈니스 로직 (프로토콜을 모름)
│   ├── auth/                          AuthCommandService, AuthQueryService
│   ├── projects/                      ProjectsCommandService, ProjectsQueryService
│   ├── consumers/                     Consumers + Subscriptions CQRS
│   ├── events/                        Pipeline: Ingest → Route → Dispatch → Outbox
│   └── delivery/                      Orchestrator + TransportRegistry + Processors
│
├── api/                             ④ 진입점 (HTTP/WS)
│   ├── rest/                          7개 컨트롤러
│   ├── dto/                           요청 DTO (Zod schema)
