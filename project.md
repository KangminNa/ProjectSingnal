...
ProjectSignal에 프로젝트를 등록하고 publisher/consumer/subscription만 설정하면 이벤트 전달 기능을 즉시 사용할 수 있어야 한다.
...
3. ProjectSignal가 온라인 사용자에게 WebSocket 전달
4. 오프라인 대상은 정책에 따라 push fallback 수행

#### 시나리오 B. 시스템 알림 전달

1. 프로젝트가 `notification.requested` 이벤트 발행
2. ProjectSignal가 routing rule 확인
...
1. client가 reconnect
2. 마지막 cursor 전달
3. ProjectSignal가 recent replay buffer에서 누락 이벤트 전달
