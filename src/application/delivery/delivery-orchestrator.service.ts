...
import { ConsumerType, ConsumerTarget } from '@domain/types/consumer.types';
import { DeliveryResult } from '@domain/types/delivery.types';
...
export class DeliveryOrchestratorService {
  ...
  private async deliverToTarget(event: EventEnvelope, target: ConsumerTarget): Promise<void> {
    ...
    const adapter = this.transportRegistry.resolve(target.consumerType);
